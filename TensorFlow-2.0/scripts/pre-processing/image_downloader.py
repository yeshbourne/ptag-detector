...
class googleimagesdownload:
    ...
    def single_image(self,image_url):
        main_directory = "ImageDownloader"
        url = image_url
        try:
        ...
        if ".jpg" in image_name or ".gif" in image_name or ".png" in image_name or ".bmp" in image_name or ".svg" in image_name or ".webp" in image_name or ".ico" in image_name:
            new_image_name = search_term +"_1."+image_name.split('.')[-1]
            output_file = open(main_directory + "/" + new_image_name, 'wb')
        else:
            new_image_name = search_term +"_1.jpg"
            output_file = open(main_directory + "/" + new_image_name + ".jpg", 'wb')

        data = response.read()
        output_file.write(data)
        response.close()
        print("completed ====> " + new_image_name)
        return
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size):
        ...
        try:
            ...
            try:
                ...
                if image_format == "":
                    image_name = search_term + "_" + str(count) + "."+ "jpg"
                else:
                    image_name = search_term + "_" + str(count) + "." + image_format
